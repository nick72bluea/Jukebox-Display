...
def create_poster(album_name, artist_name, orientation, client_id, client_secret):
    try:
        ...
        code_res = requests.get(f"https://scannables.scdn.co/uri/plain/png/000000/white/640/{uri}", timeout=5)
        ...
        if orientation == "Portrait":
            ...
            y_art = draw_wrapped_text(draw, artist_name.upper(), get_safe_font(75), (poster_w-padding)-(padding+code_w+20), poster_w-padding, code_y-12, "white")
            y_tit = draw_wrapped_text(draw, clean_name.upper(), get_safe_font(40), (poster_w-padding)-(padding+code_w+20), poster_w-padding, y_art+5, "white")
            
            # Dual Columns
            track_y_start = y_tit + 50
            f_tracks = get_safe_font(34)
            mid = (len(display_tracks) + 1) // 2
            for i, t in enumerate(display_tracks[:mid]):
                draw.text((padding, track_y_start + (i*48)), truncate_text(f"{i+1}. {t}", f_tracks, 500), font=f_tracks, fill="white")
            for i, t in enumerate(display_tracks[mid:]):
                draw.text((poster_w-padding, track_y_start + (i*48)), truncate_text(f"{t} .{mid+i+1}", f_tracks, 500), font=f_tracks, fill="white", anchor="ra")
        else:
            # Landscape implementation remains here...
