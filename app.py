...
import random
import string
import os  # FIXED: Added missing import for os.environ
...
def get_secret(key, default=None):
    ...
    try:
        # Check if key exists in st.secrets to avoid KeyError
...
"""
# Apply the Kiosk style
st.markdown(hide_st_style, unsafe_allow_html=True)

# --- CLOUD PERSISTENCE HELPERS ---
def get_saved_venue():
    return st.query_params.get("venue_id") # FIXED: Simplified access

def get_saved_display():
    return st.query_params.get("display_id") # FIXED: Simplified access
...
def clear_connection():
    # FIXED: Hardened deletion to prevent KeyErrors
    if "venue_id" in st.query_params: 
        del st.query_params["venue_id"]
    if "display_id" in st.query_params: 
        del st.query_params["display_id"]
...
if not current_venue_id or not current_display_id:
    ...
    st.markdown(f"<h1 style='text-align: center; font-size: 8rem; color: white; margin-top: -20px;'>{code[:3]} {code[3:]}</h1>", unsafe_allow_html=True)
    ...
    time.sleep(2)
    try:
        res = requests.get(f"{FIREBASE_BASE}/pairing_codes/{code}.json", timeout=5).json()
        if res and res.get("status") == "linked" and res.get("venue_id"):
            save_connection(res["venue_id"], st.session_state.temp_display_id)
            requests.delete(f"{FIREBASE_BASE}/pairing_codes/{code}.json", timeout=5)
            st.rerun() 
    except Exception: pass
    st.rerun() 
else:
    ...
    st.sidebar.markdown("---")
    def unpair_display():
        try: requests.delete(f"{FIREBASE_BASE}/venues/{current_venue_id}/displays/{current_display_id}.json", timeout=5)
        except Exception: pass
        clear_connection()
        st.rerun()
    ...
    if live_mode:
        ...
        else: st.markdown(f"<h3 style='color:gray;text-align:center;margin-top:200px;'>Listening to Venue Cloud...</h3>", unsafe_allow_html=True)
        ...
        def background_listener():
            ...
            if track_found and artist_found:
                ...
                if track_found != st.session_state.last_track or display_orientation != st.session_state.last_orientation:
                    ...
                    st.rerun()
            if time.time() - st.session_state.last_heard_time > (idle_timeout_mins * 60):
        ...
        else: st.markdown("<h3 style='color:gray;text-align:center;margin-top:200px;'>Manual Mode Active</h3>", unsafe_allow_html=True)
