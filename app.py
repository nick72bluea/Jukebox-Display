...
# --- 1. CONFIG & CREDENTIALS ---
# This setup works on Render and Streamlit Cloud
import os

# Check if we are in a local environment or cloud
try:
    SPOTIPY_CLIENT_ID = st.secrets["SPOTIPY_CLIENT_ID"]
    SPOTIPY_CLIENT_SECRET = st.secrets["SPOTIPY_CLIENT_SECRET"]
    FIREBASE_BASE = st.secrets["FIREBASE_BASE"]
except:
    # Fallback for Render or local environments using standard Env Vars
...
def get_current_song_from_cloud(venue_id):
    ...
    try:
        ...
        # If Wi-Fi drops, fail silently and return None
        pass 
    return None, None
...
def get_album_from_track(track_name, artist_name):
    try:
        ...
        # Spotify API failure or timeout
        return None
...
def create_poster(album_name, artist_name, orientation="Portrait"):
    try:
        ...
        # If anything fails (Spotify, Image Download, PIL Error), return None to keep the old poster alive
        return None
...
if not current_venue_id or not current_display_id:
    ...
    def unpair_display():
        try:
            requests.delete(f"{FIREBASE_BASE}/venues/{current_venue_id}/displays/{current_display_id}.json", timeout=5)
        except Exception: pass
        clear_connection()
